...
{
    public class PersonalImplementation : IPersonalDao
    {
        NpgsqlConnection _connection;

        public PersonalImplementation(NpgsqlConnection connection)
        {
            _connection = connection;
        }
        public async Task<List<PersonalInformationData>> GetAllPersonalInfo(string baseUri)
        {
            string query = "select * from personal.personal_info;";
                ...
                using (_connection)
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            personalInfo.ImageUrl = baseUri + reader["image_path"].ToString();
